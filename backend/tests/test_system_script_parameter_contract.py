...
class SystemScriptParameterContractTests(unittest.TestCase):
    def test_debugger_frequency_options_and_defaults_match_each_probe(self):
        configs = {item["name"]: item["default_config"] for item in SYSTEM_SCRIPT_CATALOG}

        self.assertEqual(configs["stlink_stm32_mcu_flash"]["write_speed_khz"], 900)
        self.assertEqual(
            configs["stlink_stm32_mcu_flash"]["speed_options"],
            [125, 240, 480, 900, 1800, 4000],
        )
        self.assertEqual(configs["jlink_v4_arm_mcu_flash"]["write_speed_khz"], 1000)
        self.assertEqual(
            configs["jlink_v4_arm_mcu_flash"]["speed_options"],
            [50, 100, 125, 200, 400, 500, 1000, 2000, 4000],
        )
        self.assertEqual(configs["gdlink_arm_mcu_flash"]["write_speed_khz"], 1000)
        self.assertEqual(
            configs["gdlink_arm_mcu_flash"]["speed_options"],
            [50, 100, 125, 250, 500, 1000, 2000, 4000],
        )

    def test_batch_header_uses_delayed_expansion_for_usb_binding_diagnostics(self):
        ...
        self.assertIn('echo [CONFIG] BURNER_SN=!BURNER_SN!', content)
        self.assertNotIn('echo [CONFIG] BURNER_LOCATION="%BURNER_LOCATION%"', content)
    ...
    def test_hdsc_ccid_uses_the_bundled_v604_agent_over_uart(self):
        ...
        self.assertIn('set "HDSC_BAUD_ARGS=--baud-rate %WRITE_SPEED_KHZ%"', content)
        self.assertIn('set "HDSC_BAUD_ARGS="', content)
        self.assertNotIn('set "HDSC_BAUD_ARGS=--baud-rate "%WRITE_SPEED_KHZ%""', content)
        self.assertIn('set "HDSC_ERASE_MODE_KEY=%HDSC_ERASE_MODE_KEY%"', content)
        self.assertIn('set "HDSC_COMPLETION_ACTION_KEY=%HDSC_COMPLETION_ACTION_KEY%"', content)
        self.assertNotIn('不擦除直接编程', content)
        self.assertNotIn('if "%COMPLETION_ACTION%"=="不处理"', content)
    ...
    def test_gowin_script_passes_real_programmer_cli_arguments(self):
        ...
        self.assertIn('for %%I in ("%GOWIN_PROGRAMMER_CLI%") do set "GOWIN_CLI_DIR=%%~dpI"', content)
        self.assertIn('set "PATH=%GOWIN_CLI_DIR%;%PATH%"', content)
        self.assertIn('pushd "%GOWIN_CLI_DIR%" >nul 2>nul', content)
        ...
        self.assertIn('if /I not "%GOWIN_COMPLETION_ACTION_MODE%"=="reset" exit /b 0', content)
        self.assertIn('--operation_index 1 --cable "!GOWIN_CABLE_NAME!"', content)
    ...
    def test_gowin_batch_is_included_in_the_cmd_ascii_safety_contract(self):
        ...
        self.assertIn('"hdsc_ccid_arm_mcu_flash"', tasks_source)
        self.assertIn('"altera_blaster_ii_cpld_flash"', tasks_source)
        self.assertIn('"altera_blaster_ii_fpga_flash"', tasks_source)
        self.assertIn('"xds510plus_dsp_flash"', tasks_source)
        self.assertIn('"mplab_icd3_pic_flash"', tasks_source)
    ...
    def test_batch_execution_uses_the_windows_active_code_page_without_global_chcp(self):
        ...
        self.assertIn("use_utf8_custom_batch", tasks_source)
        self.assertIn('endswith((".bat", ".cmd"))', tasks_source)
    ...
    def test_mplab_runner_uses_ascii_runtime_aliases_for_control_flow(self):
        content = build_system_script_content("mplab_icd3_pic_flash", "MPLAB ICD 3 DV164035")
        self.assertIn('set "MPLAB_ERASE_MODE_KEY=%MPLAB_ERASE_MODE_KEY%"', content)
        self.assertIn('set "MPLAB_EEPROM_WRITE_KEY=%MPLAB_EEPROM_WRITE_KEY%"', content)
        self.assertIn('set "MPLAB_COMPLETION_ACTION_KEY=%MPLAB_COMPLETION_ACTION_KEY%"', content)
        self.assertIn('set "MPLAB_BLANK_CHECK_KEY=%MPLAB_BLANK_CHECK_KEY%"', content)
        self.assertIn('set "MPLAB_EXECUTE_PROGRAM_KEY=%MPLAB_EXECUTE_PROGRAM_KEY%"', content)
        self.assertNotIn('"%IPECMD_EXE%" -TPICD3 -P%TARGET_CHIP% -E', content)
        self.assertIn('ICD3/dsPIC30 performs the full erase as part of -M', content)
        self.assertNotIn('MPLAB_USE_MDB', content)
        self.assertNotIn('Set SecureSegment.SegmentProgramming', content)
        self.assertIn('if "%ERASE_MODE%"=="全片擦除" set "MPLAB_ERASE_MODE_KEY=chip"', content)
        self.assertIn('if "%EEPROM_WRITE%"=="否" set "MPLAB_EEPROM_WRITE_KEY=no"', content)
        self.assertIn('if "%BLANK_CHECK%"=="是" set "MPLAB_BLANK_CHECK_KEY=yes"', content)
        self.assertIn('--- IPECMD 原始尾日志 ---', content)
        self.assertIn("[IPECMD-RAW]", content)
        self.assertIn('if /I "%MPLAB_EEPROM_WRITE_KEY%"=="no" set "IPECMD_PRESERVE_EE_ARG=-Z"', content)
        self.assertIn('if /I "%TARGET_CHIP:~0,3%"=="30F" set "IPECMD_PRESERVE_EE_ARG="', content)
        self.assertIn('if /I "%TARGET_CHIP:~0,7%"=="dsPIC30" set "IPECMD_PRESERVE_EE_ARG="', content)
