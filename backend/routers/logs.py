...
from backend.utils.permission import require_any_permission
...
def login_log_to_dict(log, db: Session):
    ...
    return {
        ...
        "result": log.result,
        "created_at": database_time_to_local(log.created_at),
        "updated_at": database_time_to_local(log.updated_at),
    }
...
async def get_login_logs(
    ...
    _: None = Depends(require_any_permission("log/login:view", "log:view")),
):
    """获取登录日志列表"""
...
async def clear_login_logs(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
    _: None = Depends(require_any_permission("log/login:clear", "log:clear"))
):
    """清空登录日志"""
...
async def get_operation_logs(
    ...
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
    _: None = Depends(require_any_permission("log/operation:view", "log:view")),
):
    """获取操作日志列表"""
...
async def clear_operation_logs(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
    _: None = Depends(require_any_permission("log/operation:clear", "log:clear"))
):
    """清空操作日志"""
